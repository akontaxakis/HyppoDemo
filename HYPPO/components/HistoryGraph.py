...
class HistoryGraph:
    ...
    def estimate_and_add(self, dataset, pipeline, regression_model="Gradient Boosting", split_ratio=0.3):
        ...
        artifact_graph, request = extract_artifact_graph(dataset, pipeline,X,y)
    ...
    def prune(self, K, pipelines_summary):
        ...
        pipelines_summary_true = pd.DataFrame(columns=['Pipe', 'Request', 'Choosen'])
        choosen = 0
        total_cost = 0
        while choosen < K:
            minimum_path, cost = self.choose_minimum(pipelines_summary)
    ...
    def choose_minimum(temp_history, pipelines_summary):
        requests_not_choosen_yet = pipelines_summary[pipelines_summary['Choosen'] == 0]['Request']
